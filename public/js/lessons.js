...
class Lessons {
  ...
  async getResults() {
    try {
      ...
      const t = await axios.get(`http://localhost:5000/teachers`);
...
const postLesson = async () => {
  ...
  try {
    const gl = await axios.post(`http://localhost:5000/lessons`, {
      intitule: Nom,
      teacher: Teacher
    });
...
const updateFiliere = async id => {
  ...
  try {
    const gl = await axios.patch(`http://localhost:5000/lessons/${id}`, {
      intitule: Intitule,
      teacher: Teacher
    });
  } catch (err) {
