class Option {
  ...
  async getResults() {
    try {
      ...
      const st = await axios.get(`http://localhost:5000/students`);
      const b = await axios.get(`http://localhost:5000/branches`);
      const c = await axios.get(`http://localhost:5000/lessons`);
      //const p= await axios.get(`http://localhost:5000/presences/${$('#session').val()}`);
  ...
  $(document).ready(function() {
    $('#cours').change(function() {
      ...
      document
        ...
        .insertAdjacentHTML('beforeend', `<option value="" disabled selected>Date | Heure | Salle</option>`);
      var el = $(this);
      s.forEach(element => {
        if (element.lesson == el.val()) {
          document
            ...
            .insertAdjacentHTML(
              'beforeend',
              `<option value="${element._id}">${element.date.substr(0, 10) +
                ' | ' +
                element.heure +
                ' | ' +
                element.salle}</option>`
            );
        }
      });
    });
...
const postPresence = async (e, etat) => {
  ...
  try {
    const gl = await axios.post(`http://localhost:5000/presences`, {
      studentId: e._id,
      sessionId: ssId,
      present: etat
    });
...
const getResults = async id => {
  try {
    const p = await axios.get(`http://localhost:5000/presences/${id}`);

    renderStudents(p.data.presences);
  } catch (err) {
    console.log(err);
  }
};
...
const patchPresence = async (id, etat) => {
  try {
    const gl = await axios.patch(`http://localhost:5000/presences/${id}`, {
      present: etat
    });
