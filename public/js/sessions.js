...
class Sessions {
  ...
  async getResults() {
    try {
      ...
      const l = await axios.get(`http://localhost:5000/lessons`);
...
const postStudent = async () => {
  ...
  try {
    const gl = await axios.post(`http://localhost:5000/sessions`, {
      lesson: Lecon,
      salle: Salle,
      heure: Heure,
      date: Date
...
const updateStudent = async id => {
  ...
  try {
    const gl = await axios.patch(`http://localhost:5000/sessions/${id}`, {
      salle: Salle,
      heure: Heure,
      date: Date,
      lesson: Lecon
